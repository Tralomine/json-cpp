...
string valueTypeToString(ValueType valT)
{
  switch(valT){
    case NIL:
      return "NIL";
...
Value::operator long() const
{
  ...
  if(_valueType == FLOAT)
    return int(_Float);
  else throw logic_error("bad value type requested : " + valueTypeToString(_valueType) + " instead of INT");
}
...
Value::operator double() const
{
  ...
  if(_valueType == INT)
    return double(_Int);
  else throw logic_error("bad value type requested : " + valueTypeToString(_valueType) + " instead of FLOAT");
}
...
{
  if(_valueType != BOOL)
    throw logic_error("bad value type requested : " + valueTypeToString(_valueType) + " instead of BOOL");
  return _Bool;
}
...
Value::operator string () const
{
  if(_valueType != STRING)
    throw logic_error("bad value type requested : " + valueTypeToString(_valueType) + " instead of STRING");
  return _String;
}
...
{
  if(_valueType != OBJECT)
    throw logic_error("bad value type requested : " + valueTypeToString(_valueType) + " instead of OBJECT");
  return _Object;
}
...
{
  if(_valueType != ARRAY)
    throw logic_error("bad value type requested : " + valueTypeToString(_valueType) + " instead of ARRAY");
  return _Array;
}
...
inline void skipUselessChar(string const& s, size_t& i)
{
  i = s.find_first_not_of("\t\n\r\b\f ", i);
}

json::Value json::parse(string const& string, size_t& i)
{
  ...
  if(string[i] == '{'){  //si c'est un objet
    ...
    if(string[i] != '}'){   //si l'objet continue
      do{
        ...
        std::string name("");
        if(string[i] == '\"') throw invalid_argument("malformed json exception at char " + to_string(i) + ": invalid value name");
        ...
        else throw invalid_argument("malformed json exception at char " + to_string(i) + ": missing ':'");
        skipUselessChar(string, i);
    ...
    else throw invalid_argument("malformed json exception at char " + to_string(i) + ": missing '}' or ','");
  ...
  else if(std::string("-0123456789").find(string[i]) != string::npos){ //nombres
    std::string number("");
    do{//on récup la premiere partie du nombre
      number += string[i];
      i++;
    }while(std::string("0123456789eE+-").find(string[i]) != string::npos);
    ...
    throw invalid_argument("malformed json exception at char " + to_string(i) + ": invalid value type");
  }
